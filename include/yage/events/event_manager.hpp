...
public:
  ...
  void subscribe( event_listener *listener, SDL_EventType type);
  
  void subscribe( event_listener *listener, std::vector< SDL_EventType >&& types);

  void unsubscribe( event_listener *listener, SDL_EventType type );

  void unsubscribe( event_listener *listener, std::vector< SDL_EventType >&& types ); 
  void interrupt();
  void poll_event();
  void wait_event();
private:
